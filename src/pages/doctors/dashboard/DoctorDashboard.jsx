        ...
        <div className="flex justify-between items-start">
          ...
          <div className="absolute top-20 right-0 mt-3 -ml-40 sm:hidden lg:flex">
            <img
              ...
              className="w-36 h-36 object-cover rounded-lg shadow-lg" // Adjust size and styling as needed
            />
          </div>

          {/* Content Section */}
          <div className="dashboard w-full bg-primary-100 rounded-lg sm:p-2 lg:p-5 mt-20"> {/* Added mt-20 to move content below image */}
        ...
        <div className=" grid lg:grid-cols-6 grid-cols-1 gap-6">
          {
            docdash.map((item, id)=>(
              <div key={id} className="w-full ">
                <div className="rounded-lg bg-neutral-10 p-2">
                
                  <div className="w-full flex items-center justify-between sm:px-2 lg:px-3">
                    <h2 className="text-neutral-100 font-bold capitalize">{item.title}</h2>
                    <h2 className="text-neutral-100 font-bold">{item.icon}</h2>
                  </div>
                  <div className="w-full bg-neutral-50/70 mt-5 rounded-lg sm:p-2 lg:p-3">
                    <h2 className="font-bold text-xl">{item.count}</h2>
                    <h2 className="font-bold text-neutral-100 capitalize">{item.text}</h2>
                  </div>
                </div>
              </div>
            ))

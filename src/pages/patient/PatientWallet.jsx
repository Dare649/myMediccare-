...
const PatientWallet = () => {
  ...
  const [fetchedTransactions, setFetchedTransactions] = useState([]); // renamed state
  ...
  const currentItems = Array.isArray(fetchedTransactions) ? fetchedTransactions.slice(offset, offset + itemsPerPage) : [];
  const pageCount = Math.ceil((Array.isArray(fetchedTransactions) ? fetchedTransactions.length : 0) / itemsPerPage);
  ...
  useEffect(() => {
    ...
    const fetchTransactions = async () => {
      try {
        setLoading(true);
        const response = await axiosClient.get("/api/patient/tx/all"); // Ensure this endpoint is correct
        const transactions = response?.data?.data || []; // Ensure it's an array
              ...
              <tbody className="capitalize font-normal text-lg">
                {currentItems.map((item, id) => (
                  <tr key={id} className="border-b-2 border-neutral-50 font-bold">
                    <td 
                      className={`w-20 h-20 rounded-full flex my-2 px-6 flex-row items-center justify-center text-white ${item.type === "deposit" ? "bg-green-500" : "bg-red-500"}`}>
                      <BiTransferAlt size={30}/>
                    </td>
                    <td className="py-2 px-6">{item.created_at}</td>
                    <td className={`py-2 px-6 ${item.type === "deposit" ? "text-green-500" : "text-red-500"}`}>{item.type}</td>
                    <td className={`py-2 px-6 ${item.status === "successful" ? "text-green-500" : "text-red-500"}`}>{item.status}</td>

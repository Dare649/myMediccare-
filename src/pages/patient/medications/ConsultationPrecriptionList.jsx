...
const ConsultationPrescriptionList = () => {
  ...
  useEffect(() => {
    const fetchPrescription = async () => {
      try {
        const response = await axiosClient.get(`/api/patient/appt/${id}/view_consultation`);
        setConsultation(response.data.data);
      } catch (error) {
        MySwal.fire({
          title: "Error",
          text: "Error fetching prescriptions, try again later.",
          icon: "error",
        });
      }
    };
      ...
      {consultation ? (
        <div className="w-full bg-white lg:p-10 sm:p-5">
          ...
          <div className="w-full bg-neutral-200">
            ...
            ) : consultation.prescription?.prescription_items && consultation.prescription.prescription_items.length > 0 ? (
              consultation.prescription.prescription_items.map((item, id) => (
            ...
            >
              ...
              <table className="my-8 w-full">
                ...
                <tbody className="w-full ">
                  {consultation.prescription?.prescription_items?.map((item, index) => (
                    <tr key={index} className="border-y-2 border-neutral-100 odd:bg-white text-lg">
                      <td className="py-5 capitalize">{item.drug_name}</td>
                      <td className="py-5 capitalize">
                        {item.dose} {item.dose_unit}
                      </td>
                      <td className="py-5 capitalize">{item.frequency}</td>
                      <td className="py-5 capitalize">
              ...
              <div className="py-10">
                <h2 className="capitalize text-xl">
                  prescriber: <span className="text-primary-100 font-bold">dr. {consultation.doctor_name}</span>
                </h2>
              </div>
            </div>
            ...
            <div className="p-5">
              ...
              <button
                ...
                className="sm:w-full lg:w-[50%] flex flex-col items-center justify-center mx-auto rounded-lg bg-primary-100 hover:bg-transparent text-white hover:text-primary-100 hover:border-2 hover:border-primary-100 transition-all ease-in-out duration-1000 text-lg p-2 lg:my-10 sm:my-5"
              >
        ...
        <p className="text-center font-bold text-lg">Loading consultation details...</p>
      )}
    </section>
  );
};

export default ConsultationPrescriptionList;
